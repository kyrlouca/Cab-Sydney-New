...
procedure HawbTariffsObject.RecreateHawbAllCharges();
  ...
  begin
    ...
    begin
      UpdateFactor();
      // Delete hawb item chareges first
      ksExecSQLVar(cn, 'delete from hawb_charge where fk_hawb_item is not null and fk_hawb = :hawb_serial', [FhawbSerial]);
      // *********************************************
      // We do NOT create charges for C07 OR Med
...
Function HawbTariffsObject.CreateVatHawbChargesOnItems(): integer;
  begin
    ...
    var sqlItems: String :=
           'select hi.serial_number,fk_tariff_code, hi.customs_value ,hi.vat_rate, hi.vat_code , (hi.customs_value*hi.vat_rate)/100.0 as CustomsVat from hawb_item hi where hi.FK_HAWB_SERIAL = :hawbSerial';
    ...
    var qrItems: TksQuery := TksQuery.Create(cn, sqlItems);
    ...
    Try
      ...
      begin
        ...
        begin
          ...
          vatHc.VatCode := qrItems.FieldByName('Vat_code').AsString;
          vatHc.vatRate := qrItems.FieldByName('vat_rate').AsFloat;

          vatHc.TariffUnitRate := qrItems.FieldByName('vat_rate').AsFloat;
