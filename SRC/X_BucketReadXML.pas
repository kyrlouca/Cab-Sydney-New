...
function TX_BucketreadFileFRM.CreateHawb(Const mawbSerial: integer; const hawbNode: IXMLNode; const isBucket: boolean; const FileName: string): integer;
  ...
  begin
    ...
    try
      ...
      var
      TraderRefNodes := hawbNode.ChildNodes['ShpCnsgnr'].ChildNodes;
      for var i: integer := 0 to TraderRefNodes.Count - 1 do
      begin
        var trNode: IXMLNode := TraderRefNodes[i];
        if trNode.NodeType = ntElement then
        begin
          if trNode.LocalName = 'TraderRef' then
          begin
            var agency: string := trNode.ChildNodes['TraderRefIssAgncyCd'].text.trim.ToUpper;
            if agency = 'SDT' then
            begin
              Ioss2 := trNode.ChildNodes['TraderRefId'].text;
              Break;
            end;
      ...
      qr.FieldByName('IOSS').value := Ioss2;

      /// //////////////////

      // IOSS  and Procedure code OLD
      // var Ioss: string := '';
