object S_SelectBucketFRM: TS_SelectBucketFRM
  Left = 477
  Top = 134
  BorderStyle = bsDialog
  Caption = 'S_SelectBucketHawbFRM'
  ClientHeight = 676
  ClientWidth = 1200
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 120
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1200
    Height = 65
    Align = alTop
    Caption = 'Select Bucket Hawb'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -33
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
  end
  object Panel4: TPanel
    Left = 0
    Top = 629
    Width = 1200
    Height = 47
    Align = alBottom
    TabOrder = 1
    object Panel11: TRzPanel
      Left = 1107
      Top = 1
      Width = 92
      Height = 45
      Align = alRight
      BorderOuter = fsNone
      TabOrder = 0
      Transparent = True
      object BitBtn2: TBitBtn
        Left = 11
        Top = 4
        Width = 73
        Height = 37
        Caption = 'Close'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Glyph.Data = {
          
          3F94003F94003F94003F94003F94003F94FFFFFFFFFFFFFFFFFF}
        ParentFont = False
        TabOrder = 0
        OnClick = BitBtn1Click
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1200
    Height = 40
    Align = alTop
    Color = clYellow
    ParentBackground = False
    TabOrder = 2
    object Label2: TLabel
      Left = 15
      Top = 13
      Width = 136
      Height = 16
      Caption = 'Tariff Code Or KeyWord'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object BYCodeBTN: TSpeedButton
      Left = 292
      Top = 10
      Width = 23
      Height = 24
      Hint = 'Find Hawb in this Mawb'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentFont = False
      OnClick = BYCodeBTNClick
    end
    object CodeFLD: TwwDBEdit
      Left = 165
      Top = 9
      Width = 121
      Height = 24
      AutoSize = False
      CharCase = ecUpperCase
      Font.Charset = GREEK_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Picture.PictureMaskFromDataSet = False
      Picture.PictureMask = '####  ## ## ##'
      Picture.AllowInvalidExit = True
      TabOrder = 0
      UnboundDataType = wwDefault
      WantReturns = False
      WordWrap = False
      OnKeyUp = CodeFLDKeyUp
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 105
    Width = 1200
    Height = 524
    Align = alClient
    TabOrder = 3
    object TariffGRD: TwwDBGrid
      Left = 1
      Top = 17
      Width = 992
      Height = 506
      ControlType.Strings = (
        'FK_CLEARING_STATE;CheckBox;1;0')
      Selected.Strings = (
        'HAB_ID'#9'13'#9'Hawb Id'#9'F'#9
        'DESCRIPTION'#9'37'#9'Description'#9'F'#9
        'CLEARANCE_WAITING_CODE'#9'5'#9'Status'#9'F'
        'FK_CLEARANCE_INSTRUCTION'#9'5'#9'Type'#9'F'
        'FK_CLEARING_STATE'#9'9'#9'cleared'#9'F'#9
        'INVOICE_TOTAL'#9'20'#9'Inoice Amount'#9'F'#9
        'WEIGHT'#9'10'#9'WEIGHT'#9#9
        'STATUS_C'#9'13'#9'STATUS_C'#9#9
        'FK_CUSTOMER_CODE'#9'29'#9'FK_CUSTOMER_CODE'#9#9)
      IniAttributes.Delimiter = ';;'
      IniAttributes.UnicodeIniFile = False
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      Align = alLeft
      Color = 13428976
      DataSource = BucketHawbSRC
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Calibri'
      Font.Style = []
      KeyOptions = []
      Options = [dgAlwaysShowEditor, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      TitleAlignment = taLeftJustify
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -15
      TitleFont.Name = 'Calibri'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = True
      OnTitleButtonClick = TariffGRDTitleButtonClick
      OnDblClick = TariffGRDDblClick
      OnKeyDown = TariffGRDKeyDown
      object TariffGRDIButton: TwwIButton
        Left = 0
        Top = 0
        Width = 32
        Height = 30
        AllowAllUp = True
        Flat = False
        Glyph.Data = {
          
          7C51B47C51B47C51B47C51B47C51B47C51B47C51FFFFFFFFFFFF}
        ImageIndex = -1
      end
    end
    object Panel6: TPanel
      Left = 1
      Top = 1
      Width = 1198
      Height = 16
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
    end
    object Panel5: TPanel
      Left = 993
      Top = 17
      Width = 32
      Height = 506
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 2
      Visible = False
      object Panel7: TPanel
        Left = 0
        Top = 347
        Width = 32
        Height = 159
        Align = alBottom
        BevelOuter = bvNone
        Caption = 'Panel5'
        TabOrder = 0
      end
    end
    object Panel8: TPanel
      Left = 1025
      Top = 17
      Width = 354
      Height = 506
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 3
      object AddRTariffBTN: TRzBitBtn
        Left = 0
        Top = 6
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Add Tariff'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = AddRTariffBTNClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
        Spacing = 8
      end
      object EditRBTN: TRzBitBtn
        AlignWithMargins = True
        Left = 0
        Top = 120
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Edit'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = EditRBTNClick
        Glyph.Data = {
          
          FFFFF7F6FAE1DCEBD5CCE2DDD7E8F9F8FBFFFFFFFFFFFFFFFFFF}
        Spacing = 8
      end
      object ViewRBtn: TRzBitBtn
        AlignWithMargins = True
        Left = 0
        Top = 80
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'View'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = ViewRBtnClick
        Glyph.Data = {
          
          F1F7FCFDFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Spacing = 8
      end
      object DeleteRBTN: TRzBitBtn
        AlignWithMargins = True
        Left = 0
        Top = 160
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Delete'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnClick = DeleteRBTNClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
        Spacing = 8
      end
    end
  end
  object BucketHawbSRC: TDataSource
    DataSet = BucketHawbSQL
    Left = 352
    Top = 17
  end
  object BucketHawbSQL: TIBCQuery
    SQLInsert.Strings = (
      'INSERT INTO S_TARIFF'
      '  (TARIFF_CODE, FK_TARIFF_USAGE, FK_VAT_CODE, DESCRIPTION)'
      'VALUES'
      '  (:TARIFF_CODE, :FK_TARIFF_USAGE, :FK_VAT_CODE, :DESCRIPTION)')
    SQLDelete.Strings = (
      'DELETE FROM S_TARIFF'
      'WHERE'
      '  TARIFF_CODE = :Old_TARIFF_CODE')
    SQLUpdate.Strings = (
      'UPDATE S_TARIFF'
      'SET'
      
        '  TARIFF_CODE = :TARIFF_CODE, FK_TARIFF_USAGE = :FK_TARIFF_USAGE' +
        ', FK_VAT_CODE = :FK_VAT_CODE, DESCRIPTION = :DESCRIPTION'
      'WHERE'
      '  TARIFF_CODE = :Old_TARIFF_CODE')
    SQLRefresh.Strings = (
      
        'SELECT TARIFF_CODE, FK_TARIFF_USAGE, FK_VAT_CODE, DESCRIPTION FR' +
        'OM S_TARIFF'
      'WHERE'
      '  TARIFF_CODE = :TARIFF_CODE')
    SQLLock.Strings = (
      'SELECT NULL FROM S_TARIFF'
      'WHERE'
      'TARIFF_CODE = :Old_TARIFF_CODE'
      'FOR UPDATE WITH LOCK')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM ('
      'SELECT 1 AS C  FROM S_TARIFF'
      ''
      ') q')
    Connection = ClairDML.CabCOnnection
    SQL.Strings = (
      'SELECT SERIAL_NUMBER,  a.HAB_ID, '
      'a.FK_CUSTOMER_CODE, a.DESCRIPTION, a.WEIGHT, a.INVOICE_TOTAL,'
      
        '    a.STATUS_C, a.FK_CLEARING_STATE,FK_CLEARANCE_INSTRUCTION,CLE' +
        'ARANCE_WAITING_CODE'
      'FROM HAWB a ;')
    Active = True
    Left = 248
    Top = 9
    object BucketHawbSQLHAB_ID: TStringField
      DisplayLabel = 'Hawb Id'
      DisplayWidth = 13
      FieldName = 'HAB_ID'
      Required = True
      Size = 10
    end
    object BucketHawbSQLDESCRIPTION: TStringField
      DisplayLabel = 'Description'
      DisplayWidth = 37
      FieldName = 'DESCRIPTION'
      Size = 200
    end
    object BucketHawbSQLCLEARANCE_WAITING_CODE: TStringField
      DisplayLabel = 'Status'
      DisplayWidth = 5
      FieldName = 'CLEARANCE_WAITING_CODE'
      FixedChar = True
      Size = 5
    end
    object BucketHawbSQLFK_CLEARANCE_INSTRUCTION: TStringField
      DisplayLabel = 'Type'
      DisplayWidth = 5
      FieldName = 'FK_CLEARANCE_INSTRUCTION'
      FixedChar = True
      Size = 5
    end
    object BucketHawbSQLFK_CLEARING_STATE: TStringField
      DisplayLabel = 'cleared'
      DisplayWidth = 9
      FieldName = 'FK_CLEARING_STATE'
      Required = True
      FixedChar = True
      Size = 1
    end
    object BucketHawbSQLINVOICE_TOTAL: TFloatField
      DisplayLabel = 'Inoice Amount'
      DisplayWidth = 20
      FieldName = 'INVOICE_TOTAL'
    end
    object BucketHawbSQLWEIGHT: TFloatField
      DisplayWidth = 10
      FieldName = 'WEIGHT'
    end
    object BucketHawbSQLSTATUS_C: TStringField
      DisplayWidth = 13
      FieldName = 'STATUS_C'
      FixedChar = True
      Size = 1
    end
    object BucketHawbSQLFK_CUSTOMER_CODE: TIntegerField
      DisplayWidth = 29
      FieldName = 'FK_CUSTOMER_CODE'
    end
    object BucketHawbSQLSERIAL_NUMBER: TIntegerField
      FieldName = 'SERIAL_NUMBER'
      Required = True
      Visible = False
    end
  end
  object ReadTrans: TIBCTransaction
    DefaultConnection = ClairDML.CabCOnnection
    IsolationLevel = iblReadOnlyReadCommitted
    Left = 24
    Top = 9
  end
  object WriteTrans: TIBCTransaction
    DefaultConnection = ClairDML.CabCOnnection
    Left = 97
    Top = 5
  end
  object ChargingMethodSQL: TIBCQuery
    GeneratorMode = gmInsert
    SQLInsert.Strings = (
      'INSERT INTO CHARGING_METHOD'
      '  (CHARGING_CODE, DESCRIPTION, IS_DEFAULT)'
      'VALUES'
      '  (:CHARGING_CODE, :DESCRIPTION, :IS_DEFAULT)')
    SQLDelete.Strings = (
      'DELETE FROM CHARGING_METHOD'
      'WHERE'
      '  CHARGING_CODE = :Old_CHARGING_CODE')
    SQLUpdate.Strings = (
      'UPDATE CHARGING_METHOD'
      'SET'
      
        '  CHARGING_CODE = :CHARGING_CODE, DESCRIPTION = :DESCRIPTION, IS' +
        '_DEFAULT = :IS_DEFAULT'
      'WHERE'
      '  CHARGING_CODE = :Old_CHARGING_CODE')
    SQLRefresh.Strings = (
      
        'SELECT CHARGING_CODE, DESCRIPTION, IS_DEFAULT FROM CHARGING_METH' +
        'OD'
      'WHERE'
      '  CHARGING_CODE = :CHARGING_CODE')
    SQLLock.Strings = (
      'SELECT NULL FROM CHARGING_METHOD'
      'WHERE'
      'CHARGING_CODE = :Old_CHARGING_CODE'
      'FOR UPDATE WITH LOCK')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM ('
      'SELECT 1 AS C  FROM CHARGING_METHOD'
      ''
      ') q')
    Connection = ClairDML.CabCOnnection
    Transaction = ReadTrans
    UpdateTransaction = WriteTrans
    SQL.Strings = (
      'SELECT'
      '*'
      'From '
      'Charging_method')
    ReadOnly = True
    Left = 649
    Top = 21
    object ChargingMethodSQLCHARGING_CODE: TStringField
      FieldName = 'CHARGING_CODE'
      Required = True
      FixedChar = True
      Size = 3
    end
    object ChargingMethodSQLDESCRIPTION: TStringField
      FieldName = 'DESCRIPTION'
      FixedChar = True
      Size = 15
    end
    object ChargingMethodSQLIS_DEFAULT: TStringField
      FieldName = 'IS_DEFAULT'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
end
