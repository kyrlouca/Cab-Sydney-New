...
procedure TTransferBuckets.TransferOneBucketHawb(bucketSerial: integer; origin: string);
  begin
    ...
    var
    sqlExisting := 'select   serial_number, hab_id,NUM_OF_PIECES_ARRIVED ,FK_CLearing_state,FK_INVOICE_STATUS from hawb ha  where ha.hab_id=:hawbId and ha.fk_mawb_refer_number= :mawbSerial;';
    ...
    Try
      ...
      //DO NOT Transfer the bucket if there is an invoiced or cleared Hawb
      var isExistingREady := qrExisting.FieldByName('FK_CLEARING_STATE').AsString= '1';
      var isExistingInvoiced := qrExisting.FieldByName('FK_INVOICE_STATUS').AsString= '1';
      if isExistingInvoiced or isExistingREady then begin

        exit;

      end;

      //if there is NO existing hawb just change the Mawb Reference
      //if a hawb EXISTS, then delete it and then change the bucket
      //a hawb may not exists in case the user just wants to manually transfer a bucket to a Mawb
...
Function TTransferBuckets.SendToPendingLow(hawbSerial: integer; origin: string): boolean;
  begin
  //create records in table Pending_low for the hawbs that are cleared
