...
type
  ...
  TB_BucketHawbDataDML = class(TDataModule)
...
var
  B_BucketHawbDataDML: TB_BucketHawbDataDML;
...
procedure TB_BucketHawbDataDML.DataModuleCreate(Sender: TObject);
begin
cn:=ClairDML.CabCOnnection;
end;


procedure TB_BucketHawbDataDML.HawbChargeCustomsSQLTARIFF_UNIT_RATEGetText(
...
procedure TB_BucketHawbDataDML.HawbCommentSQLAfterPost(DataSet: TDataSet);
begin
     Dataset.FieldByName('The_Comments').ReadOnly:=True;

end;

procedure TB_BucketHawbDataDML.HawbCommentSQLBeforeEdit(DataSet: TDataSet);
begin
     Dataset.FieldByName('The_Comments').ReadOnly:=True;

end;

procedure TB_BucketHawbDataDML.HawbCommentSQLNewRecord(DataSet: TDataSet);
begin
...
procedure TB_BucketHawbDataDML.HawbSQLBeforeEdit(DataSet: TDataSet);
begin
//If (Dataset.FieldByName('FK_INVOICE_STATUS').AsString>'0')
If (Dataset.FieldByName('FK_INVOICE_STATUS').AsString='- Does not apply for bucket')
then begin
...
procedure TB_BucketHawbDataDML.HawbSQLBeforeInsert(DataSet: TDataSet);
begin
   Dataset.FieldByName('hab_id').ReadOnly:=False;

end;

procedure TB_BucketHawbDataDML.HawbSQLNewRecord(DataSet: TDataSet);
var
  qr:TksQuery;
  inco:String;
begin
...
procedure TB_BucketHawbDataDML.SenderInvoiceSQLBeforeEdit(DataSet: TDataSet);
Var
 HawbDS:TIBCQuery;
begin
  HawbDS:= HawbSQL;
...
//  If (HawbDS.FieldByName('FK_INVOICE_STATUS').AsString>'0') then begin
  If (HawbDS.FieldByName('FK_INVOICE_STATUS').AsString='- does not apply for Bucket') then begin
     MessageDlg('Can NOT Modify Invoiced Data ',mtInformation, [mbOk], 0);
     abort;
