...
procedure TV_HawbFRM.ShowUser1Click(Sender: TObject);
  begin
    var userId: string := v_hawbDataDML.HawbSQL.FieldByName('User_cleared').AsString;
    var  userName :string :=ksGetOneFieldValue(cn,'select user_name from security_user usr where usr.user_id= :userId ','user_Name' ,[userId]);
    MessageDlg('Hawb Cleared by : ' + userId+' - '+userName, TMsgDlgType.mtInformation, [mbOK], 0);
  end;
...
procedure TV_HawbFRM.ClearBTNClick(Sender: TObject);
  ...
  begin
    ...
    var user: string := MainFFF.Global_UserID;

    var sqlUpdUser: string := 'update hawb hb set hb.user_cleared= :user where hb.serial_number= :serial';
    ksExecSQLVar(cn, sqlUpdUser, [user, HawbSerial]);
