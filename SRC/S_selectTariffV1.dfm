object S_SelectTariffV1FRM: TS_SelectTariffV1FRM
  Left = 477
  Top = 134
  BorderStyle = bsDialog
  Caption = 'S_SelectTariffV1FRM'
  ClientHeight = 676
  ClientWidth = 1146
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 120
  TextHeight = 16
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1146
    Height = 65
    Align = alTop
    Caption = 'Select Tariff'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -33
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
  end
  object Panel4: TPanel
    Left = 0
    Top = 629
    Width = 1146
    Height = 47
    Align = alBottom
    TabOrder = 1
    object Panel11: TRzPanel
      Left = 1053
      Top = 1
      Width = 92
      Height = 45
      Align = alRight
      BorderOuter = fsNone
      TabOrder = 0
      Transparent = True
      object BitBtn2: TBitBtn
        Left = 11
        Top = 4
        Width = 73
        Height = 37
        Caption = 'Close'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        Glyph.Data = {
          
          3F94003F94003F94003F94003F94003F94FFFFFFFFFFFFFFFFFF}
        ParentFont = False
        TabOrder = 0
        OnClick = BitBtn1Click
      end
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 65
    Width = 1146
    Height = 40
    Align = alTop
    TabOrder = 2
    object Label2: TLabel
      Left = 15
      Top = 13
      Width = 136
      Height = 16
      Caption = 'Tariff Code Or KeyWord'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
    end
    object BYCodeBTN: TSpeedButton
      Left = 292
      Top = 10
      Width = 23
      Height = 24
      Hint = 'Find Hawb in this Mawb'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        333333773FF77333333333370007333333333333777333333333}
      NumGlyphs = 2
      ParentFont = False
      OnClick = BYCodeBTNClick
    end
    object CodeFLD: TwwDBEdit
      Left = 165
      Top = 9
      Width = 121
      Height = 24
      AutoSize = False
      CharCase = ecUpperCase
      Font.Charset = GREEK_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      Picture.PictureMaskFromDataSet = False
      Picture.PictureMask = '####  ## ## ##'
      Picture.AllowInvalidExit = True
      TabOrder = 0
      UnboundDataType = wwDefault
      WantReturns = False
      WordWrap = False
      OnKeyUp = CodeFLDKeyUp
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 105
    Width = 1146
    Height = 524
    Align = alClient
    TabOrder = 3
    object TariffGRD: TwwDBGrid
      Left = 1
      Top = 17
      Width = 800
      Height = 506
      ControlType.Strings = (
        'IS_LOW;CheckBox;Y;N')
      Selected.Strings = (
        'TARIFF_CODE'#9'16'#9'Tariff'#9#9
        'FK_TARIFF_USAGE'#9'7'#9'Usage'#9#9
        'IS_LOW'#9'1'#9'Medium'#9'F'#9
        'THE_KEY'#9'17'#9'Key'#9#9
        'DESCRIPTION'#9'24'#9'Description'#9#9
        'EXCISE'#9'10'#9'Excise'#9#9
        'IMPORT'#9'10'#9'Import'#9#9
        'FK_VAT_CODE'#9'3'#9'Vat'#9#9
        'VAT'#9'10'#9'Vat Rate'#9#9)
      IniAttributes.Delimiter = ';;'
      IniAttributes.UnicodeIniFile = False
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      Align = alLeft
      Color = 13428976
      DataSource = TariffSRC
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Arial'
      Font.Style = []
      KeyOptions = []
      Options = [dgAlwaysShowEditor, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgWordWrap]
      ParentFont = False
      ReadOnly = True
      TabOrder = 0
      TitleAlignment = taLeftJustify
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -16
      TitleFont.Name = 'Arial'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = True
      OnTitleButtonClick = TariffGRDTitleButtonClick
      OnDblClick = TariffGRDDblClick
      OnKeyDown = TariffGRDKeyDown
      object TariffGRDIButton: TwwIButton
        Left = 0
        Top = 0
        Width = 32
        Height = 30
        AllowAllUp = True
        Flat = False
        Glyph.Data = {
          
          7C51B47C51B47C51B47C51B47C51B47C51B47C51FFFFFFFFFFFF}
        ImageIndex = -1
      end
    end
    object Panel6: TPanel
      Left = 1
      Top = 1
      Width = 1144
      Height = 16
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
    end
    object Panel5: TPanel
      Left = 801
      Top = 17
      Width = 23
      Height = 506
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 2
      Visible = False
      object Panel7: TPanel
        Left = 0
        Top = 347
        Width = 23
        Height = 159
        Align = alBottom
        BevelOuter = bvNone
        Caption = 'Panel5'
        TabOrder = 0
      end
    end
    object Panel8: TPanel
      Left = 824
      Top = 17
      Width = 330
      Height = 506
      Align = alLeft
      BevelOuter = bvNone
      TabOrder = 3
      object AddRTariffBTN: TRzBitBtn
        Left = 6
        Top = 102
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Add Tariff'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = AddRTariffBTNClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
        Spacing = 8
      end
      object EditRBTN: TRzBitBtn
        AlignWithMargins = True
        Left = 6
        Top = 216
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Edit'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = EditRBTNClick
        Glyph.Data = {
          
          FFFFF7F6FAE1DCEBD5CCE2DDD7E8F9F8FBFFFFFFFFFFFFFFFFFF}
        Spacing = 8
      end
      object ViewRBtn: TRzBitBtn
        AlignWithMargins = True
        Left = 6
        Top = 176
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'View'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = ViewRBtnClick
        Glyph.Data = {
          
          F1F7FCFDFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Spacing = 8
      end
      object DeleteRBTN: TRzBitBtn
        AlignWithMargins = True
        Left = 6
        Top = 256
        Width = 107
        Height = 34
        Alignment = taLeftJustify
        Caption = 'Delete'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Arial'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnClick = DeleteRBTNClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
        Spacing = 8
      end
      object Memo1: TMemo
        Left = 127
        Top = 50
        Width = 74
        Height = 95
        Lines.Strings = (
          'Memo1')
        TabOrder = 4
        Visible = False
      end
      object HighFilter: TRzRadioGroup
        Left = 6
        Top = 6
        Width = 107
        Height = 65
        Caption = 'Show Tariffs'
        Font.Charset = GREEK_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Arial'
        Font.Style = []
        ItemHeight = 14
        ItemIndex = 0
        Items.Strings = (
          'High'
          'Medium')
        ParentFont = False
        TabOrder = 5
        VerticalSpacing = 6
        OnChanging = HighFilterChanging
      end
    end
  end
  object TariffSRC: TDataSource
    DataSet = TariffSQL
    Left = 400
    Top = 9
  end
  object TariffSQL: TIBCQuery
    SQLInsert.Strings = (
      'INSERT INTO S_TARIFF'
      
        '  (TARIFF_CODE, FK_TARIFF_USAGE, FK_VAT_CODE, DESCRIPTION, IS_LO' +
        'W)'
      'VALUES'
      
        '  (:TARIFF_CODE, :FK_TARIFF_USAGE, :FK_VAT_CODE, :DESCRIPTION, :' +
        'IS_LOW)')
    SQLDelete.Strings = (
      'DELETE FROM S_TARIFF'
      'WHERE'
      '  TARIFF_CODE = :Old_TARIFF_CODE')
    SQLUpdate.Strings = (
      'UPDATE S_TARIFF'
      'SET'
      
        '  TARIFF_CODE = :TARIFF_CODE, FK_TARIFF_USAGE = :FK_TARIFF_USAGE' +
        ', FK_VAT_CODE = :FK_VAT_CODE, DESCRIPTION = :DESCRIPTION, IS_LOW' +
        ' = :IS_LOW'
      'WHERE'
      '  TARIFF_CODE = :Old_TARIFF_CODE')
    SQLRefresh.Strings = (
      
        'SELECT TARIFF_CODE, FK_TARIFF_USAGE, FK_VAT_CODE, DESCRIPTION, I' +
        'S_LOW FROM S_TARIFF'
      'WHERE'
      '  TARIFF_CODE = :TARIFF_CODE')
    SQLLock.Strings = (
      'SELECT NULL FROM S_TARIFF'
      'WHERE'
      'TARIFF_CODE = :Old_TARIFF_CODE'
      'FOR UPDATE WITH LOCK')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM ('
      'SELECT 1 AS C  FROM S_TARIFF'
      ''
      ') q')
    Connection = ClairDML.CabCOnnection
    SQL.Strings = (
      
        'SELECT first 40 a.TARIFF_CODE,kk.the_key,a.is_low, a.fk_tariff_u' +
        'sage,a.description,  a.FK_VAT_CODE, a.DESCRIPTION,a.FK_TARIFF_US' +
        'AGE,'
      
        '(select vat.rate from vat_category vat where vat.code= a.fk_vat_' +
        'code)as VAT,'
      
        '(select max(line.tariff_unit_rate) from s_tariff_line line  wher' +
        'e line.fk_s_tariff_code= a.tariff_code and line.duty_type='#39'IMP'#39' ' +
        ')as Import,'
      
        '(select max(line.tariff_unit_rate) from s_tariff_line line  wher' +
        'e line.fk_s_tariff_code= a.tariff_code and line.duty_type='#39'EXC'#39' ' +
        ')as Excise'
      'from S_Tariff a'
      
        'left outer join s_tariff_key kk on a.tariff_code=kk.fk_s_tariff_' +
        'code')
    Active = True
    Left = 296
    Top = 9
    object TariffSQLTARIFF_CODE: TStringField
      DisplayLabel = 'Tariff'
      DisplayWidth = 16
      FieldName = 'TARIFF_CODE'
      Required = True
      FixedChar = True
      Size = 17
    end
    object TariffSQLFK_TARIFF_USAGE: TStringField
      DisplayLabel = 'Usage'
      DisplayWidth = 7
      FieldName = 'FK_TARIFF_USAGE'
      FixedChar = True
      Size = 3
    end
    object TariffSQLIS_LOW: TStringField
      DisplayLabel = 'Medium'
      DisplayWidth = 1
      FieldName = 'IS_LOW'
      FixedChar = True
      Size = 1
    end
    object TariffSQLTHE_KEY: TStringField
      DisplayLabel = 'Key'
      DisplayWidth = 17
      FieldName = 'THE_KEY'
      ReadOnly = True
      FixedChar = True
    end
    object TariffSQLDESCRIPTION: TStringField
      DisplayLabel = 'Description'
      DisplayWidth = 24
      FieldName = 'DESCRIPTION'
      FixedChar = True
      Size = 60
    end
    object TariffSQLEXCISE: TFloatField
      DisplayLabel = 'Excise'
      DisplayWidth = 10
      FieldName = 'EXCISE'
      ReadOnly = True
      DisplayFormat = ',##0.00;;#'
    end
    object TariffSQLIMPORT: TFloatField
      DisplayLabel = 'Import'
      DisplayWidth = 10
      FieldName = 'IMPORT'
      ReadOnly = True
      DisplayFormat = ',##0.00;;#'
    end
    object TariffSQLFK_VAT_CODE: TStringField
      DisplayLabel = 'Vat'
      DisplayWidth = 3
      FieldName = 'FK_VAT_CODE'
      Required = True
      FixedChar = True
      Size = 3
    end
    object TariffSQLVAT: TFloatField
      DisplayLabel = 'Vat Rate'
      DisplayWidth = 10
      FieldName = 'VAT'
      ReadOnly = True
      DisplayFormat = ',##0.00;;#'
    end
    object TariffSQLDESCRIPTION_1: TStringField
      FieldName = 'DESCRIPTION_1'
      Visible = False
      FixedChar = True
      Size = 60
    end
    object TariffSQLFK_TARIFF_USAGE_1: TStringField
      FieldName = 'FK_TARIFF_USAGE_1'
      Visible = False
      FixedChar = True
      Size = 3
    end
  end
  object ReadTrans: TIBCTransaction
    DefaultConnection = ClairDML.CabCOnnection
    IsolationLevel = iblReadOnlyReadCommitted
    Left = 24
    Top = 9
  end
  object WriteTrans: TIBCTransaction
    DefaultConnection = ClairDML.CabCOnnection
    Left = 97
    Top = 5
  end
  object ChargingMethodSQL: TIBCQuery
    GeneratorMode = gmInsert
    SQLInsert.Strings = (
      'INSERT INTO CHARGING_METHOD'
      '  (CHARGING_CODE, DESCRIPTION, IS_DEFAULT)'
      'VALUES'
      '  (:CHARGING_CODE, :DESCRIPTION, :IS_DEFAULT)')
    SQLDelete.Strings = (
      'DELETE FROM CHARGING_METHOD'
      'WHERE'
      '  CHARGING_CODE = :Old_CHARGING_CODE')
    SQLUpdate.Strings = (
      'UPDATE CHARGING_METHOD'
      'SET'
      
        '  CHARGING_CODE = :CHARGING_CODE, DESCRIPTION = :DESCRIPTION, IS' +
        '_DEFAULT = :IS_DEFAULT'
      'WHERE'
      '  CHARGING_CODE = :Old_CHARGING_CODE')
    SQLRefresh.Strings = (
      
        'SELECT CHARGING_CODE, DESCRIPTION, IS_DEFAULT FROM CHARGING_METH' +
        'OD'
      'WHERE'
      '  CHARGING_CODE = :CHARGING_CODE')
    SQLLock.Strings = (
      'SELECT NULL FROM CHARGING_METHOD'
      'WHERE'
      'CHARGING_CODE = :Old_CHARGING_CODE'
      'FOR UPDATE WITH LOCK')
    SQLRecCount.Strings = (
      'SELECT COUNT(*) FROM ('
      'SELECT 1 AS C  FROM CHARGING_METHOD'
      ''
      ') q')
    Connection = ClairDML.CabCOnnection
    Transaction = ReadTrans
    UpdateTransaction = WriteTrans
    SQL.Strings = (
      'SELECT'
      '*'
      'From '
      'Charging_method')
    ReadOnly = True
    Active = True
    Left = 649
    Top = 21
    object ChargingMethodSQLCHARGING_CODE: TStringField
      FieldName = 'CHARGING_CODE'
      Required = True
      FixedChar = True
      Size = 3
    end
    object ChargingMethodSQLDESCRIPTION: TStringField
      FieldName = 'DESCRIPTION'
      FixedChar = True
      Size = 15
    end
    object ChargingMethodSQLIS_DEFAULT: TStringField
      FieldName = 'IS_DEFAULT'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
end
